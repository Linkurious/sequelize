...
var QueryGenerator = {
  ...
  changeColumnQuery: function(tableName, attributes) {
    ...
    // we assume that changeColumnQuery is used to change only one column
    // This is a workaround till sequelize v4
    ...
    for (var attributeName in attributes) {
      ...
      var indexOfDefault = splittedDefinition.indexOf('DEFAULT');
      if (indexOfDefault >= 0) {
        defaultValue = splittedDefinition[indexOfDefault + 1];
        if (defaultValue.indexOf ('N\'') === 0) {
          defaultValue = defaultValue.slice(1);
        }

        //constraintString.push(`DEFAULT ${splittedDefinition[indexOfDefault + 1]}`);
        splittedDefinition.splice(indexOfDefault, 2);
        definition = splittedDefinition.join(' ');
      }
    ...
    if (attrString.length) {
      columnName = attrString[0].split(' ')[0];
    ...
    return Utils._.template(query)({
      tableName: this.quoteTable(tableName),
      query: finalQuery
    }) + (defaultValue !== undefined ? ` ALTER table [${tableName}] ` +
